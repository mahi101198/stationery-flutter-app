...
export const onOrderStatusUpdated = onDocumentUpdated(
  ...
  async (event) => {
    try {
      ...
      // ── FCM ────────────────────────────────────────────────────────────────
      if (statusChanged) {
        const isRazorpayWebhookTransition =
          beforeStatus === 'processing_payment' && afterStatus === 'confirmed';
        ...
        if (isRazorpayWebhookTransition) {
          ...
          logger.info(
            `[${requestId}] ⏭️  Skipping FCM for ${beforeStatus} → ${afterStatus} — webhook will send payment_success notification`
          );
      ...
      if (statusChanged && beforeStatus !== 'delivered' && afterStatus === 'delivered') {
        ...
        try {
          const orderAmount = afterData.paymentSummary?.totalOrderValue || 
                            afterData.pricingSummary?.totalBeforePayment || 0;
...
async function getFcmToken(userId: string): Promise<string | null> {
  ...
  if (!token) logger.warn(`No FCM token for user ${userId}`);
  return token;
}
 ...
 */
async function sendStatusFCMNotification(orderId: string, orderData: any, status: string, requestId: string) {
  try {
 ...
 */
async function sendDeliveryConfirmationNotification(orderId: string, orderData: any, requestId: string) {
  try {
    const userId = orderData.userId;
 ...
 */
export const sendManualDeliveryConfirmation = async (orderId: string) => {
  try {
    ...
    if (orderData?.status !== 'delivered') {
      throw new Error(`Order ${orderId} is not delivered (status: ${orderData?.status})`);
    }
    if (orderData?.deliveryConfirmed) {
